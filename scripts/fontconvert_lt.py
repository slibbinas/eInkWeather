#!python3
# Modifikuotas LilyGo-EPD47 fontconvert.py — prideti lietuvisku raidziu intervalai.
# Naudojimas: python fontconvert_lt.py OpenSans12B 12 C:\Windows\Fonts\segoeuib.ttf --compress > ..\include\opensans12b.h
import freetype
import zlib
import sys
import math
...
intervals = [
    (0x20, 0x7E),    # ASCII
    (0xA0, 0xFF),    # Latin-1 (deg. zenklas ir kt.)
    (0x104, 0x105),  # Ą ą
    (0x10C, 0x10D),  # Č č
    (0x116, 0x119),  # Ė ė Ę ę
    (0x12E, 0x12F),  # Į į
...
for i, g in enumerate(glyph_props):
    ch = chr(g.code_point)
    comment = ch if (0x20 <= g.code_point <= 0x7E and g.code_point != 92) else f"U+{g.code_point:04X}"
    print("    { " + ", ".join([f"{a}" for a in list(g[:-1])]), "},", f"// {comment}")
print("};")
